package  
{
	...
	public class Lacuna extends Sprite
	{
		...
		private var manUpdate:Boolean = true;
		...
		private function buildWorld():void {
			...
			for each(var wall in xml.normalworld.staticobjs.staticobj)
			{
				...
				{
					//trace("making killer");
					allKillers.push(new killer(this, new Point(parseInt(wall.@x), parseInt(wall.@y)), parseInt(wall.@length), parseInt(wall.@dir), killer.SPIKES, parseInt(wall.@inverted)));
				}
			}
			...
			/*rgtPanel = new panel();
			rgtPanel.x = STAGE.stageWidth;
			rgtPanel.y = STAGE.stageHeight;
			STAGE.addChild(rgtPanel);*/
			...
			l1p.scaleX = l1p.scaleY = 0.4;
			mc.addChild(l1p);
			rgtPanel.addChild(mc);
			mc.x = -rgtPanel.width + 50;
			mc.y = -STAGE.stageHeight + 50;*/
			
			/*cluMenu = new ClueMenu(numClues, 1);
			rgtPanel.addChild(cluMenu);
			cluMenu.x = -rgtPanel.width + 50;
			cluMenu.y = -120;*/
			
			/*for each(var pipe in xml.pipes.pipe) {
				l1p.newPipe(parseInt(pipe.@x), parseInt(pipe.@y), new String(pipe.@type), new String(pipe.@inverted), new String(pipe.@expected), new String(pipe.@expectedcolor), parseInt(pipe.@initial));
			}*/
		...
		private function createBoundingWalls():void {
			...
			new Wall(this, new Point( 0, -1), GenConstants.getWidthUnits(), 0, false, 1, 0.7);
			new Wall(this, new Point( GenConstants.getWidthUnits(), 0), GenConstants.getHeightUnits(), 1, false, 1, 0.7);
			new Wall(this, new Point( 0, GenConstants.getHeightUnits()), GenConstants.getWidthUnits(), 0, false, 1, 0.7);
		}
		...
		public function toggleView(pt:b2Vec2):void {
			...
			if (worldView) {
				...
				else if (pt.y > 800 / GenConstants.RATIO) {
					originY = 600;
				}
				else {
		...
		public function shootMan(theta:Number):void {
			...
			if (theta > 0 && theta <= 90) {
				theta = toRad(theta);
				velVec = new b2Vec2(Math.floor(GenConstants.CANNON_IMPULSE * Math.cos(theta)), Math.floor(GenConstants.CANNON_IMPULSE * Math.sin(theta)));
			}
			else if (theta > 90 && theta <= 180) {
				theta -= 90;
				theta = toRad(theta);
				trace(theta);
				velVec = new b2Vec2(Math.floor(-GenConstants.CANNON_IMPULSE * Math.sin(theta)), Math.floor(GenConstants.CANNON_IMPULSE * Math.cos(theta)));
			}
